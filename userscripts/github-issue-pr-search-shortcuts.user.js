...
(function () {
  ...
  function parseIssueCountFromHtml(html) {
    if (!html) return null;
  ...
  async function fetchSearchCount(url) {
    ...
    const promise = (async () => {
      ...
      const count = parseIssueCountFromHtml(html);
      if (!Number.isFinite(count)) throw new Error("Unable to parse issueCount from search HTML");
